...
def afficher_espace_eleve():
    ...
    with col_s1:
        nom_rech = st.text_input("Nom", placeholder="Ex : MBALLA",
                                  key="eleve_nom", value=st.session_state.get("eleve_nom_recherche",""))
    with col_s2:
    ...
    if chercher and nom_rech.strip():
        ...
        # ── Section 1 : Résultats des tests psychotechniques ──
        st.markdown("---")
        st.markdown("### 🧪 Résultats des Tests Psychotechniques")

        TESTS_INFO = {
            "d48_brut":  ("D48 — Raisonnement logique",  "d48_etal"),
            "krx_brut":  ("KRX — Aptitude mathématique", "krx_etal"),
            "meca_brut": ("MECA — Compréhension mécanique","meca_etal"),
            "bv11_brut": ("BV11 — Vocabulaire littéraire","bv11_etal"),
            "prc_brut":  ("PRC — Proverbes & raisonnement","prc_etal"),
        }
        cols_t = st.columns(5)
        for col, (brut_key, (nom_test, etal_key)) in zip(cols_t, TESTS_INFO.items()):
            brut = float(d.get(brut_key) or 0)
            etal = float(d.get(etal_key) or brut)
            code = brut_key.replace("_brut","").upper()
            col.markdown(f"""
            <div class="score-card">
                <div class="score-label">{code}</div>
                <div class="score-raw">Note brute : {brut:.1f}</div>
                <div class="score-etalon">{etal:.1f}/20</div>
                <div style="font-size:0.65rem;opacity:0.6;margin-top:2px;">{nom_test.split("—")[0].strip()}</div>
            </div>""", unsafe_allow_html=True)
    ...
    elif not nom_rech.strip():
        st.markdown("""
        <div class="alert-info">
            👆 Saisie ton <strong>nom de famille</strong> dans le champ ci-dessus et clique
            sur <strong>Rechercher</strong> pour accéder à ton dossier personnel.
        </div>""", unsafe_allow_html=True)
        ...
        if not mode_conseiller:
            ...
            with c1:
                ...
                for lbl, key in [("Mathématiques", f"maths{suffix}"),
                    ...
                    st.markdown(f"**{lbl} :** `{val:.1f}/20`")
                moy = (st.session_state.get(f"maths{suffix}",0)+
                       st.session_state.get(f"sci_phy{suffix}",0)+
                       st.session_state.get(f"svt{suffix}",0))/3
            ...
            with c2:
                ...
                for lbl, key in [("Français", f"francais{suffix}"),
                    ...
                    val = st.session_state.get(key, 0.0)
                    st.markdown(f"**{lbl} :** `{val:.1f}/20`")
                moy = (st.session_state.get(f"francais{suffix}",0)+
                       st.session_state.get(f"histgeo{suffix}",0)+
                       st.session_state.get(f"anglais{suffix}",0))/3
...
# =====================================================================
    ...
    if st.session_state.get("notes_conseiller",""):
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);
                    border-left:5px solid #f59e0b;border-radius:12px;
