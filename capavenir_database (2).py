...
class CapAvenirDB:
    ...
    def sauvegarder_dossier(self, ss, etalonner_fn=None) -> dict:
        ...
        def etal(v):
            if etalonner_fn:
                return etalonner_fn(v)
            # Étalonnage linéaire simplifié si fonction non fournie
            return round(min(20.0, max(0.0, v * 1.0)), 2)
        ...
        d48  = float(_get("d48",  10.0))
        krx  = float(_get("krx",  10.0))
        meca = float(_get("meca", 10.0))
        bv11 = float(_get("bv11", 10.0))
        prc  = float(_get("prc",  10.0))

        SA_brut = (krx + d48) / 2
        LA_brut = (bv11 + prc) / 2
        SA_etal = (etal(krx) + etal(d48)) / 2
        LA_etal = (etal(bv11) + etal(prc)) / 2

        # ── Notes scolaires ──
        def notes_trim(t):
            """Retourne les notes et moyennes d'un trimestre donné."""
            maths   = float(_get(f"maths_{t}",   0.0))
            sci_phy = float(_get(f"sci_phy_{t}", 0.0))
            svt     = float(_get(f"svt_{t}",     0.0))
            francais= float(_get(f"francais_{t}",0.0))
            histgeo = float(_get(f"histgeo_{t}", 0.0))
            anglais = float(_get(f"anglais_{t}", 0.0))
            moy_sci = round((maths + sci_phy + svt) / 3, 2)
            moy_lit = round((francais + histgeo + anglais) / 3, 2)
            return {
        ...
        if table == "orientations_confirmees":
            data = {
                **base,
                ...
                "d48_etal": etal(d48), "krx_etal": etal(krx), "meca_etal": etal(meca),
                "bv11_etal": etal(bv11), "prc_etal": etal(prc),
            ...
            data = {
                **base,
                "serie_cible": serie,
                "serie_risque": "A" if serie == "C" else "C",
                "statut": "probation",
                "objectif_moy_sci": obj_sci,
                "objectif_moy_lit": obj_lit,
